...
# Очистка позиции
@dp.callback_query_handler(text_startswith="position_clear_this", state="*")
...
@dp.callback_query_handler(text_startswith="yes_clear_position", state="*")
async def open_category_for_clear_position(call: CallbackQuery, state: FSMContext):
    position_id = int(call.data.split(":")[1])
    category_id = int(call.data.split(":")[2])
    remover = int(call.data.split(":")[3])
...
@dp.callback_query_handler(text_startswith="not_clear_position", state="*")
async def open_category_for_clear_position(call: CallbackQuery, state: FSMContext):
    position_id = int(call.data.split(":")[1])
    category_id = int(call.data.split(":")[2])
    remover = int(call.data.split(":")[3])
...
@dp.callback_query_handler(text_startswith="back_add_item_to_category", state="*")
